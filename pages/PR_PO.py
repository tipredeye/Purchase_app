...
# LOAD DATA
# ------------------------------------------------------------
df_req = load_sheet("Request")      # จะว่างก็ได้ ไม่เป็นไร
...
# ------------------------------------------------------------
# BULK ACTION: แก้ไขสถานะใน PR_PO
# ------------------------------------------------------------
...
if df_prpo.empty:
    ...
    if df_prpo_view.empty:
        ...
        edited_prpo = st.data_editor(
            ...
            num_rows="fixed",
            key="prpo_status_editor_bulk",   # <<< key ใหม่ ไม่ซ้ำ
        )
        ...
        if do_bulk:
            ...
            if not selected_index:
                st.error("กรุณาติ๊กแถวที่ต้องการเปลี่ยนสถานะก่อน")
            else:
...
# ------------------------------------------------------------
# 3) รายการใบสั่งซื้อ (PO) + รับเข้าสินค้า
# ------------------------------------------------------------
...
# data_editor สำหรับแก้ Quantity_Received
...
edited_po_view = st.data_editor(
    ...
    disabled=disabled_cols,
    num_rows="fixed",
    key="po_receive_editor",        # <<< key ไม่ซ้ำ
)
...
# ----- บันทึกจากตาราง (รับเข้าเป็นราย row) -----
